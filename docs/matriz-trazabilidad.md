...
Estado actual: RF01 a RF05 implementados, más la infraestructura de log de
eventos e integración con Sentry. RF06 a RF11 tienen sus casos de prueba
escritos y marcados como `xfail` a la espera de su implementación.

RF05 pasa sus ocho casos de aceptación, pero la prueba cruzada encontró tres
defectos abiertos, #27, #28 y #29, que aparecen cuando las reglas se combinan
entre varias solicitudes en vez de dentro de una sola. Sus casos, CP-50 a
CP-52, están escritos y marcados `xfail(strict=True)`: al corregirse el
defecto, el caso pasa y `strict` obliga a quitar la marca.
...
| RF05 | La solicitud nace en `por_revisar` con 1 o 2 equipos de categorías distintas, dentro del máximo de 7 días y sobre equipos disponibles | `servicios/solicitudes.py` | CP-24 a CP-31, CP-53, CP-54, CP-55 | Aprobado con defectos abiertos (#27, #28, #29) |
...
| RN01 | Un solicitante no puede tener más de 2 equipos simultáneamente, sumando todas sus solicitudes activas | `servicios/solicitudes.py` | CP-50 | **Fallido**, defecto #27 |
| RN02 | Los equipos en poder de una persona son de categorías distintas, aunque vengan de solicitudes separadas | `servicios/solicitudes.py` | CP-51 | **Fallido**, defecto #28 |
